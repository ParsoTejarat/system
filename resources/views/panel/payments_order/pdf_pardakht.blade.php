...
<div class="box9">{{$orderPayment->number}} </div>
<div class="box8">{{$date}}</div>


<div class="box1"> لطفا مبلغ :{{number_format($orderPayment->amount)}} ریال</div>
...
<div class="box5"> طی فاکتور شماره :{{$orderPayment->invoice_number??0}} پرداخت گردد.</div>
@if($orderPayment->is_online_payment == 1)
    <div class="box6">پرداخت اینترنتی سایت :{{$orderPayment->site_name??'--'}} </div>
@else
    <div class="box6"> شماره حساب / شبا :{{$orderPayment->bank_number??'--'}} </div>
