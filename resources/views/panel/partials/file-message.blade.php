    ...
    <ul class="w-100">
        <li class="flex-column justify-content-center align-items-center w-100">
            ...
            <a href="{{ env('API_PATH_URL').$file->path }}">
                <img src="{{ env('API_PATH_URL').$file->path }}" alt="image" class="w-100">
                <br>
                <span>{{ $file->name }}</span>
            </a>
        </li>
...
@else
    <div class="m-b-0 text-muted text-left media-file">
        <a href="{{ env('API_PATH_URL').$file->path }}" class="btn btn-outline-light row text-left align-items-center justify-content-center h-50 w-100" download="{{ env('API_PATH_URL').$file->path }}">
