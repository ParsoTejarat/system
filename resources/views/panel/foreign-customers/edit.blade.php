...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('foreign-customers.update', $foreignCustomer->id) }}" method="post"
                ...
                <div class="form-row">
                    ...
                    <div class="col-xl-3 col-lg-3 col-md-3 mb-3">
                        <label for="products">
                            <i class="fa fa-question-circle text-info" data-toggle="tooltip" data-placement="bottom"
                               data-original-title="پس از تایپ نام محصول Enter را بزنید" style="font-size: large"></i>
                            محصولات
                        </label>
                        <input type="text" name="products" class="form-control tagsinput"
                               value="{{ $foreignCustomer->products }}">
                        @error('products')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-3 mb-3">
                        ...
                        <textarea name="description" id="description"
                                  class="form-control">{{ $foreignCustomer->description }}</textarea>
                        @error('description')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-3 mb-3">
                        ...
                        @if($foreignCustomer->docs)
                            @foreach(json_decode($foreignCustomer->docs) as $key => $doc)
                                <a href="{{ $doc }}" target="_blank"><span
                                            class="badge badge-info"> فایل{{ ++$key }} </span></a>
                            @endforeach
                        @endif
                    </div>
                </div>
