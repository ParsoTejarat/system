    ...
    <form action="" method="post">
        <div class="form-row">
            ...
            <table class="table table-bordered mb-5" style="margin-top: 2rem">
                ...
                <tbody>
                    <tr style="padding-top: 2rem">
                        @if(!is_null($transfer->recipient_name))
                        <td>
                            <span><strong>نام گیرنده:</strong> {{ $transfer->recipient_name }}</span>
                        </td>
                        @endif
                    </tr>
                    <tr>
                        @if(!is_null($transfer->address))
                        <td>
                            <span><strong>نشانی:</strong> {{ $transfer->address }}</span>
                        </td>
                        @endif
                    </tr>
                    <tr>
                        @if(!is_null($transfer->zip_code))
                        <td>
                            <span><strong>کد پستی:</strong> {{ $transfer->zip_code }}</span>
                        </td>
                        @endif
                    </tr>
                    <tr>
                        @if(!is_null($transfer->phone))
                        <td>
                            <span><strong>شماره تلفن:</strong> {{ $transfer->phone }}</span>
                        </td>
                        @endif
                    </tr>
                </tbody>
